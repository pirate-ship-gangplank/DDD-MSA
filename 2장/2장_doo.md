...
객체의 상태 변화를 이벤트 메세지로 발행하고, 또 객체 상태를 관계형 데이터베이스에 저장하는 경우, SQL 질의어로 변환해서 처리하기가 매우 번거롭고 까다롭다.

또한 메세지 발행과 데이터 저장 처리라는 2가지 기능을 수행하기 때문에 빠르지도 않다.

메세지 발행 및 저장 처리의 원자성을 보장하고 성능도 최적화하는 방법이 있다.
