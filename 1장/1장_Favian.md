...
User 를 담당하는 애플리케이션과 DB가 있고,
쇼핑을 담당하는 애플리케이션과 DB가 있다.

사용자 인증은 AccessToken 으로 하고 있다.

이 경우에 사용자가 쇼핑을 하는 도중에 인증이 필요한 부분이 있다면

쇼핑 애플리케이션 -> User 애플리케이션 -> User DB

이렇게 통신을 하는 건가?

- 큐 메커니즘

이벤트 큐를 위한 어떠한 애플리케이션 서버가 또 필요한건가?

비동기는 여러 작업을 동시에 수행할 수 있는 이점이 있는데

이벤트 큐에 비동기를 적용하면 어떠한 이점이 있는건지 이해가 잘 가지는 않는다
